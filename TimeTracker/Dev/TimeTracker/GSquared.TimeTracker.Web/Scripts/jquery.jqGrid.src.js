...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setColWidth();
		$(eg).css("width",(grid.width+55)+"px").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+ts.p.id+"'>&#160;</div>");
		$(gv).css("width", (grid.width + 55) + "px");
		...
		if(ts.p.pager){
			...
			$(ts.p.pager).css({width: (grid.width+55)+"px"}).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom').appendTo(eg);
		...
		$(grid.bDiv)
			...
			.css({ height: ts.p.height+(isNaN(ts.p.height)?"":"px"), width: (grid.width+55)+"px"})
